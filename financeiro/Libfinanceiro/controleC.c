...
#include <stdlib.h>
#include <string.h>
#include "contasAReceber.h"  // Incluindo o módulo de contas a receber
...
float obterSaldoCaixa() {
    ...
    if (formatoReg == 1) { // Arquivo em formato binário
...
// Função para registrar o pagamento em dinheiro ou débito no formato texto
void registrarPagamentoTxt(int codigo, const char *nomeCliente, int tipoPagamento, float valor) {
    ...
    time_t dataPagamento = time(NULL); // Data atual

    fprintf(caixaTxt, "%d|%s|%d|+%.2f|%ld\n", codigo, nomeCliente, tipoPagamento, valor > 0 ? valor : +valor, dataPagamento);
    fclose(caixaTxt);
...
void registrarPagamento() {
    ...
    if (formatoReg == 1) {
        registrarPagamentoBin(codigo, nomeCliente, tipoPagamento, valor);
    } else {
        registrarPagamentoTxt(codigo, nomeCliente, tipoPagamento, valor);
    }
}
...
void registrarPagamentoCartaoCredito(int codigo, float valor) {
    ...
    // Solicitar os dados da conta a receber reutilizando a lógica de entrada
    regContaReceber(); // Essa função já coleta todos os dados necessários e chama `regContaReceberPath`.

    // Substituir os campos específicos para pagamento com cartão de crédito
    conta.codigo = codigo;
    conta.valor = valor;

    // Registrar a conta utilizando regContaReceberPath (já embutido na função regContaReceber)
}
...
void somarcaixaBin(int codigoConta, const char *nomeCliente, float valor, time_t dataRecebimento) {
    ...
    registro.codigo = codigoConta;
    strcpy(registro.nomeCliente, nomeCliente);
    registro.tipoPagamento = 4;  // Tipo de pagamento 4, como no exemplo
    registro.valor = valor;      // Valor positivo, já que estamos adicionando ao caixa
